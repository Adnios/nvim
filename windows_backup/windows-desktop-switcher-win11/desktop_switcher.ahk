...
MoveAndBackCurrentWindowToDesktop(desktopNumber) {
    global CurrentDesktop, DesktopCount
...
MoveAndBackCurrentWindowToRightDesktop()
{
    global CurrentDesktop, DesktopCount
    updateGlobalVariables()
    WinGet, activeHwnd, ID, A
...
MoveAndBackCurrentWindowToLeftDesktop()
{
    global CurrentDesktop, DesktopCount
    updateGlobalVariables()
    WinGet, activeHwnd, ID, A
...
Loop 2  ; <-- 增加这个值来加快滚动速度.
    SendMessage, 0x0114, 1, 0, %fcontrol%, A  ; 0x0114 是 WM_HSCROLL, 它后面的 0 是 SB_LINELEFT.
return

~LShift & WheelUp::
ControlGetFocus, fcontrol, A
Loop 2  ; <-- 增加这个值来加快滚动速度.
    SendMessage, 0x0114, 0, 0, %fcontrol%, A  ; 0x0114 是 WM_HSCROLL, 它后面的 1 是 SB_LINERIGHT.
return

IsWindowsWalker := false
Alt::
    If (A_PriorHotKey == A_ThisHotKey and A_TimeSincePriorHotkey < 1000){
        Send !{Backspace}
        Sleep, 100
        Send {Down}
        IsWindowsWalker := true
    }
    return
#If (IsWindowsWalker)
    ^n::Down
...
; I don't like this
IsAltTabMenu := false
; single alt to open AltTab
; LAlt & f::
;     Send !^{Tab}
;     IsAltTabMenu := true
;     return
; double alt to open AltTab
; Space::
;     If (A_PriorHotKey == A_ThisHotKey and A_TimeSincePriorHotkey < 1000){
;         Send !^{Tab}
;         IsAltTabMenu := true
